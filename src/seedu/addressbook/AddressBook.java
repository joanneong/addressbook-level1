...
public class AddressBook {
     ...
     * The details are further stored in an enumeration, and accessed without the use of a string array.
     */
    private enum PersonProperty  {NAME, EMAIL, PHONE};

    private static final PersonProperty PERSON_PROPERTY_NAME = PersonProperty.NAME;
    private static final PersonProperty PERSON_PROPERTY_PHONE = PersonProperty.PHONE;
    private static final PersonProperty PERSON_PROPERTY_EMAIL = PersonProperty.EMAIL;
    ...
    private static void showWelcomeMessage() {
        String[] msg = {DIVIDER, DIVIDER, VERSION, MESSAGE_WELCOME, DIVIDER};
        showToUser(msg);
    }

    private static void showResultToUser(String result) {
        String[] res = {result, DIVIDER};
        showToUser(res);
    }
    ...
    private static void echoUserCommand(String userCommand) {
        String[] command = {"[Command entered:" + userCommand + "]"};
        showToUser(command);
    }
    ...
    private static void processProgramArgs(String[] args) {
        if (args.length >= 2) {
            String[] msg = {MESSAGE_INVALID_PROGRAM_ARGS};
            showToUser(msg);
            exitProgram();
        }
    ...
    private static void setupGivenFileForStorage(String filePath) {
        ...
        if (!isValidFilePath(filePath)) {
            String[] msg = {String.format(MESSAGE_INVALID_FILE, filePath)};
            showToUser(msg);
            exitProgram();
        }
    ...
    private static void exitProgram() {
        String[] exit = {MESSAGE_GOODBYE, DIVIDER, DIVIDER};
        showToUser(exit);
        System.exit(0);
    }
    ...
    private static void setupDefaultFileForStorage() {
        String[] msg = {MESSAGE_USING_DEFAULT_FILE};
        showToUser(msg);
    ...
    */
    ...
    private static void createFileIfMissing(String filePath) {
        ...
        String[] err = {String.format(MESSAGE_ERROR_MISSING_STORAGE_FILE, filePath)};
        showToUser(err);

        try {
            storageFile.createNewFile();
            String[] msg = {String.format(MESSAGE_STORAGE_FILE_CREATED, filePath)};
            showToUser(msg);
        } catch (IOException ioe) {
            String[] error = {String.format(MESSAGE_ERROR_CREATING_STORAGE_FILE, filePath)};
            showToUser(error);
            exitProgram();
        }
    }
    ...
    private static ArrayList<HashMap<PersonProperty, String>> loadPersonsFromFile(String filePath) {
        ...
        if (!successfullyDecoded.isPresent()) {
            String[] invalid = {MESSAGE_INVALID_STORAGE_FILE_CONTENT};
            showToUser(invalid);
            exitProgram();
        }
    ...
    private static ArrayList<String> getLinesInFile(String filePath) {
        ...
        try {
            ...
            String[] msg = {String.format(MESSAGE_ERROR_MISSING_STORAGE_FILE, filePath)};
            showToUser(msg);
            exitProgram();
        } catch (IOException ioe) {
            String[] error = {String.format(MESSAGE_ERROR_READING_FROM_FILE, filePath)};
            showToUser(error);
            exitProgram();
        }
        return lines;
    ...
    private static void savePersonsToFile(ArrayList<HashMap<PersonProperty, String>> persons, String filePath) {
        ...
        try {
            ...
            String[] err = {String.format(MESSAGE_ERROR_WRITING_TO_FILE, filePath)};
            showToUser(err);
            exitProgram();
        }
    }
    ...
    private static HashMap<PersonProperty, String> makePersonFromData(String name, String phone, String email) {
        final HashMap<PersonProperty, String> person = new HashMap<>(PERSON_DATA_COUNT);
    ...
    private static boolean isPersonPhoneValid(String phone) {
        return phone.matches("\\d+");    // phone sequence of exactly 8 digits (assume local numbers)
        //TODO: implement a more permissive validation
    }
